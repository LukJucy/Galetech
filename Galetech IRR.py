...
class GaletechAssetOptimizer:
    ...
    def evaluate_combination(self, t_model, t_count, s_mw, b_mwh, rep_days, return_traces=False, wind_shock=1.0, ppa_shock=1.0):
        annual_revenue = 0
        annual_co2_saved = 0
        annual_btm_supply_kwh = 0
        annual_curtailed_kwh = 0
        annual_curtail_cost = 0
        ...
        for day_idx, day in enumerate(rep_days):
            ...
            p_solar = s_mw * 1000 * day['solar_irrad']
...
def load_custom_typical_days(df=None, custom_weights=None):
    ...
    if df is not None:
        ...
        t = np.arange(24)
        rep_days.append({'wind_speed': np.clip(3 + 2*np.sin(t/4), 0, None), 'solar_irrad': np.clip(np.sin((t-6)*np.pi/12), 0, 1), 'elec_load': 1200 + 400*np.sin((t-8)*np.pi/12), 'gas_load': 200 + 50*np.random.rand(24), 'weight': weights[0] if len(weights)>0 else 90})
        rep_days.append({'wind_speed': np.clip(8 + 3*np.cos(t/4), 0, None), 'solar_irrad': np.clip(0.4 * np.sin((t-8)*np.pi/8), 0, 1), 'elec_load': 800 + 200*np.sin((t-6)*np.pi/12), 'gas_load': 2000 + 800*np.cos((t-12)*np.pi/12), 'weight': weights[1] if len(weights)>1 else 90})
        rep_days.append({'wind_speed': np.clip(5 + 2*np.sin(t/6), 0, None), 'solar_irrad': np.clip(0.7 * np.sin((t-6)*np.pi/12), 0, 1), 'elec_load': 900 + 150*np.sin(t/4), 'gas_load': 600 + 100*np.random.rand(24), 'weight': weights[2] if len(weights)>2 else 185})
    return rep_days

# ==========================================
# 3. Streamlit UI 界面与报告生成
# ==========================================
...
if run_btn:
    ...
    optimizer_params = {
        ...
        'cost_solar_mw': c_solar_mw, 'cost_bess_mwh': c_bess_mwh, 'civil_solar': cv_solar, 'civil_bess': cv_bess,
    ...
    with st.spinner("Executing Mathematical Optimization (Co-optimising sizing, export & curtailment)..."):
        # 修改循环：允许完全没有发电设备只有电池的情景，满足 benchmarking 需求
            ...
            for t_count in counts:
                for s in range(0, max_solar + 1, max(1, max_solar//5)): 
                    for b in range(0, max_bess + 1, max(1, max_bess//5)): 
                        ...
                        # -------------------------------------------------------------
                        # ✅ 修复点：完美解包 5 个变量，彻底解决 ValueError!
                        # -------------------------------------------------------------
                        ...
                        res_list.append({
                            ...
                            'Curtailed_MWh': curtail_kwh/1000, 'Curtail_Cost_k': curtail_cost/1000, # 加入弃电追踪
    ...
    st.session_state['best_config'] = best # 缓存最优解给 Monte Carlo 用
    ...
    with tab4:
        ...
        if st.button("🎲 Run 50 Monte Carlo Simulations on Optimal Config"):
            ...
            for i in range(50):
                # 随机生成扰动因子 (Shocks)
                wind_shock = np.random.normal(1.0, 0.10) # 风资源波动 ±10%
                capex_shock = np.random.normal(1.0, 0.05) # CAPEX超支波动 ±5%
                ppa_shock = np.random.normal(1.0, 0.05)   # 客户电价波动 ±5%
                ...
                # -------------------------------------------------------------
                # ✅ 修复点：Monte Carlo 循环同样完美解包 5 个变量！
                # -------------------------------------------------------------
            ...
            with c1:
                ...
                p90, p50, p10 = np.percentile(df_mc['NPV10_M'], [10, 50, 90]) # 财务中P90代表有90%概率超过该值(偏保守)
